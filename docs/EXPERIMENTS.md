...
| **Recuperación combinada con "DINOv3" (en realidad DINOv2), 2026-08-30** | ❌ Descartado — regresión grande y significativa vs. producción. **Aviso crítico encontrado antes de pilotar**: los cinco scripts existentes (`harvest_dinov3.py`, `eval_dinov3_*.py`, `reembed_dinov3.py`) cargan `timm.create_model('vit_base_patch14_dinov2.lvd142m', ...)` — es DINOv2, no DINOv3, pese al nombre en todos los ficheros; no hay ningún checkpoint DINOv3 real en el entorno. Además la extracción usó solo 2 fotos/especie (`PER_SP=2`) y guarda un único prototipo, no una galería k-NN por imagen — ya identificado como "prototipos insuficientes" en el cierre negativo previo de DINOv2 en solitario (51.78%, −19.9pp vs ViT-H, re-confirmado en una v2 corregida tras un bug real de cruce de IDs Minka/iNat). Se piloteó igualmente con estos datos (rotulados correctamente como DINOv2 en todo el log/salida) sobre el subconjunto alineado del oficial 77.77% (8,796/12,788 obs cuya especie verdadera tiene prototipo DINOv2). Resultados: DINOv2 solo 52.74% (confirma el −19.9pp histórico); BioCLIP-prototipo-solo (control deliberadamente débil, sin kNN/geo/rerank) 62.01%; oficial real en ese subconjunto 75.88%; fusión BioCLIP+DINOv2 (peso calibrado por 5-fold OOF, α=0.8 en los 5 folds) 69.12%. La comparación contra el control débil parece positiva (+782/−156, 5.01:1, p<0.0001) pero es un espejismo: cualquier señal adicional parchea un BioCLIP artificialmente degradado sin kNN ni rerank. La comparación que importa, contra el oficial real, es clara: **75.88%→69.12% (−6.75pp)**, 354 arreglados / 948 rotos (0.37:1), McNemar exacto χ²=270.084, **p<0.0001 — regresión grande y significativa**. Sin cutover. |
| **Multi-prototipos morfológicos + covarianza local, Tier1 (≥15 refs/especie), 2026-08-30** | ❌ Descartado — sin señal real, no significativo. Hipótesis: el prototipo único de producción (centroide de todas las referencias) puede diluir variación intra-específica real (dimorfismo, fases juveniles, poses) en especies con suficientes fotos para que eso importe; sub-clusterizar por k-means y usar la similitud MÁXIMA a cualquiera de los sub-prototipos podría recuperar esa señal. Umbral ≥15 refs cubre 2,811/4,709 especies pero 12,766/12,788 obs (99.8%) del eval set — no aísla una élite, es el mínimo práctico para que k-means con k≤4 tenga sentido. 2,811 especies sub-clusterizadas (silhouette score elige k∈{2,3,4} por especie, mayoría k=2), 6,682 sub-prototipos totales, mezclados con el prototipo único vía un peso BLEND calibrado por 5-fold OOF sobre {0, 0.25, 0.5, 0.75, 1.0}, reutilizando `exp_hierarchical_query_cache.npz` (cero GPU nueva). **4 de 5 folds eligieron BLEND=0** (sin cambio) como óptimo. McNemar exacto vs. control propio (ablación limpia, global n=12,788 y Tier1 n=12,766 dan el mismo resultado ya que casi coinciden): 3 arreglados / 7 rotos (0.43:1), χ²=0.900, **p=0.3438 — no significativo**. El prototipo global ya captura lo necesario para el boost de producción; separar en sub-clusters morfológicos no aporta señal medible. Sin cutover. |
