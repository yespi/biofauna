...
- Input-side cleanup (central-crop / multi-crop region-of-interest embeddings) as a follow-up to the closed taxonomic-consensus experiment above: if Bucket C errors stem from background/substrate contaminating the *encoder input* rather than a fixable ranking artifact, cleaning the crop before BioCLIP-2.5 sees it — rather than re-ranking a possibly-contaminated embedding after the fact — is the next candidate direction (in progress)
