...
> **2026-08-29** · Official calibration harvest, full corpus: **77.08% species** with ROI multi-crop fusion + Bucket B Fisher-diagonal re-ranking (τ=0.20) + adaptive prototype boost by local k-NN margin (+0.16pp net on top of the 76.92% Bucket B harvest, n=12,788, geo prior included). `biofauna-id.service` **was restarted the same day, with explicit authorization, to bring ROI fusion + Bucket B live** (serving 76.78%/76.92% depending on which calibration snapshot is loaded); the newest mechanism (adaptive prototype boost) and its 77.08% calibration are shipped in code but **await a further restart**, pending explicit go-ahead. Also live: **76.76% species with multi-photo late fusion** on the older crop90 baseline — the *combined* full-corpus accuracy of ROI fusion + multi-photo fusion + Bucket B + adaptive prototype stacked has not yet been independently re-measured, see note below  
