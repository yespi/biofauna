...
| **+ Bucket B Fisher-diagonal re-ranking, current** (Aug 28-29 2026, n=12,788, single photo) | **76.92%** | +0.14pp net on top of the 76.78% ROI-fusion official harvest (geo included); confidence-gated (τ=0.20) top-1/top-2 swap for documented same-genus cryptic pairs, 58 fixed / 41 broken; genus 81.88%, family 85.53%. Code shipped to `identify_service.py`, calibration re-harvested and re-fit. **`biofauna-id.service` not yet restarted**, so this is not yet the live-served figure. See paper §4.11 |

Do not compare these numbers without noting corpus size and embedding coverage. The 76.84% ROI-fusion row above measures an isolated no-geo ablation baseline; the 76.78%/76.92% figures in this note and the Bucket B row are the official geo-inclusive `calibration.json` harvest. The multi-photo, ROI-fusion, and Bucket B rows were each measured against their **own** baseline in isolation (Bucket B against the ROI-fusion official harvest specifically) — all now shipped to production code, but the combined full-corpus number with all three stacked has not yet been independently re-measured.
