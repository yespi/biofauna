...
1. **Re-embebido de la galería con ViT-H a escala de catálogo completo** para fauna marina mediterránea — **785.897** embeddings en **4.702** especies objetivo a 1-sep-2026 (762.082 / 4.709 en la base TTA del 27-ago), unificando un almacén SSD+archivo y densificando después los taxones con pocas fotos (§4.15).
...
9. **Un invariante operativo para índices de recuperación** (§4.16): tras un `/reload` en caliente, el índice FAISS y sus `species_ids` alineados por fila deben cargarse juntos; reconstruir solo el array de etiquetas en memoria desde el `embeddings.npy` actual mapea vecinos correctos a nombres de especie incorrectos.
10. **Densificación en staging con McNemar overlay** (§4.17): se pueden puntuar vectores extra de galería contra el FAISS de producción como overlay mmap CPU (un índice por proceso) sin cutover vivo; el N/M overlay-vs-control no es el Δ del identify vivo si la réplica omite Cubo B / geo.
...
### 4.15 Densificación del catálogo sin un encoder nuevo (31-ago a 1-sep-2026)

Tras congelar la pila de inferencia de §4.9–§4.14 en **77,77%** de especie (n=12.788) — cinco hipótesis arquitectónicas más no devolvieron ganancia significativa — el error restante se atacó con **más fotos de referencia**, no con más maquinaria. Cosechas dirigidas a especies con pocas fotos (limpieza de grupos de color Tier3, lotes Tier1 sucesivos, luego un lote masivo) se re-embebieron con el mismo ViT-H congelado y se reconstruyeron en FAISS. Producción tras el lote masivo: **785.897** embeddings / **4.702** especies.

`harvest_calib` oficial sobre la misma cohorte n=12.788: **79,25%** especie / **75,10%** Tier1. La ganancia es densificación de la galería, no un scorer nuevo. Un incremento nocturno sobre 726 especies adicionales (índice staging **807.267**) midió **79,10%** global (−0,15pp, McNemar p=0,053) pese a un **+1,73pp** en el slice de esas 726 (p=0,014): interferencia k-NN contra vecinas no tocadas. Ese índice staging **no** se promocionó. Un reponderado `score / log1p(n_ref)` también se descartó (−0,07pp). Tratamos **79,25% / 785.897** como la base oficial en vivo, y 807.267 como artefacto a no promover.

Un experimento de recorte del query (YOLO26n COCO, pad=0,25) puntuó **78,74%** sobre el mismo conjunto oficial — por debajo del 79,25% — así que el recorte PRE por defecto de FotoFauna sigue siendo YOLO-COCO, no un recorte YOLOE con prompts marinos (recupera cajas marinas pero baja el acierto de especie de BioFauna tras el recorte, 78,3%→52–56% en un harvest marino de 23 fotos).

A 3-sep-2026 la fuente operativa de verdad es el jsonl `calib_raw.jsonl`: **10.115 / 12.788 = 79,10%** especie (Tier1 **74,88%**, n=7.282). El 79,25% es el recuento de la autopsia 1-sep del mismo harvest (Δ19 fotos). Conservamos 79,25% como fila del harvest de densificación y **79,10% jsonl** como baseline oficial vivo.

### 4.16 Desincronización FAISS / array de etiquetas en producción (detectada 1-sep-2026, corregida)

**Síntoma.** Durante unas 34 horas (31-ago ~12:03 a 1-sep 22:42 CEST), el `/identify` vivo asignó **nombres de especies marinas al 85–100% de confianza calibrada a fotografías terrestres** (p. ej. una jara identificada como *Chthamalus stellatus*). Las publicaciones AutoID que usaron el servicio vivo en esa ventana no son fiables; los trabajos de evaluación basados en disco no se vieron afectados.

**Causa.** El proceso FastAPI había cargado un índice FAISS de **777.544** vectores al arrancar. Llamadas posteriores a `POST /reload` reconstruyeron el array de etiquetas en memoria `KY` desde el `embeddings.npy` de entonces (layout de **785k filas**) **sin recargar FAISS**. La recuperación devolvía los vecinos correctos del índice viejo y los traducía con **nombres de fila incorrectos**. Reproducción offline de la observación 75314: índice 777k alineado + `species_ids.npy` coincidente → *Cistus albidus* (sim≈0,94); las mismas filas FAISS con `KY` del layout 785k → *Chthamalus stellatus*, igual que en producción. No es un intercambio de IDs de taxón Minka/iNaturalist ni un re-embebido corrupto: `embeddings.npy` en disco siguió alineado con `species_ids.npy`.

**Por qué las métricas oficiales siguen valiendo.** `harvest_calib`, las pasarelas McNemar, la comparación 79,25% / 79,10% staging, y el experimento de recorte pad=0,25 abren FAISS **desde disco** con `species_ids.npy`. Nunca consultaron el `KY` vivo. Las ablaciones de ajuste fino tampoco llaman a `:8090`.

**Arreglo (en producción).** La traducción de vecinos usa el `species_ids.npy` que acompaña a FAISS (`_faiss_y`), no `KY`. `load_faiss()` rechaza una carga si `len(ids) != ntotal`. `/reload` ahora llama a `load_faiss()`. `/health` expone `faiss_ntotal`, `faiss_ids` y `faiss_aligned`. Tras el reinicio: `faiss_ntotal=785897`, `faiss_ids=785897`, `faiss_aligned=true`. Seis observaciones Minka terrestres que habían sido imanes marinos recuperaron sus nombres botánicos/terrestres; 49/50 aciertos de la calibración oficial siguieron coincidiendo con el top-1 vivo.

La lección es operativa, no arquitectónica: **un índice de recuperación y sus etiquetas de fila son un solo artefacto**. Recargar prototipos en caliente sin el índice es un modo de fallo silencioso y de alta confianza.

### 4.17 Overlay de densificación en staging (night85, 3–4 sep 2026, sin cutover)

El identify vivo sigue en **79,10%** jsonl sobre FAISS **785.897** (`faiss_aligned=true`). Una cosecha reversible de especies close-call con pocas fotos (iNaturalist + Minka, **JPGs locales del archive primero**, embed vía `POST /embed`) escribe vectores extra en `exp_density_rank2/` y **no** muta `dataset/faiss_index/` ni `patterns/`.

El McNemar es un **overlay**: un FAISS de producción mmap en CPU más los vectores extra, fusión del top-15 global, y el mismo voto k=15 + prototipo adaptativo de una réplica en disco (**sin** Cubo B / geo vivos). Versus esa réplica como control (4-sep ~11:59 CEST): **282 arreglados / 80 rotos**, **+1,58 pp**, p exacto ≪0,01, fix/break **3,53**. Versus el jsonl oficial la misma réplica está ~2,5 pp por debajo del identify vivo, así que ese N/M **no** es métrica de promoción. Un «techo» maxsim (~152 fotos / +1,19 pp) **tampoco** es la ganancia de acierto medida.

La cosecha de la especie *N+1* corre en paralelo con el embed GPU de la especie *N*. El overlay se relanza cada ~12 spp densificadas, siempre un FAISS por proceso. **Sin cutover**: aunque overlay-vs-control cumple p / ratio / +0,3 pp, el scorer no es el identify vivo. Fichero de parada: `night85_STOP`.

### 4.18 Refresco de galería sensible a calidad y temperatura de agregación kNN (9–10 sep 2026)

Reparación completa de fotos con desincronización embedding-disco (508 especies, 9.824+3.967 fotos en 229+30 especies primero, luego re-embed no incremental completo de las 508), seguido de un FAISS de staging (838.115 vectores, bajando de 927.890 — menos fotos pero mejor puntuadas) que supera al índice de producción pre-reparación en un McNemar con corte anticipado (n=2.500): **+2,16 pp** (77,44%→79,60%), fix/break 105/51, ratio 2,06, p=2,2e-05 — cutover ejecutado.

Independientemente del contenido de la galería, el agregador de puntuación kNN por clase cambió de una suma llana de similitudes coseno positivas a una exponencial tipo softmax, `Σ exp(max(s,0)/T)` con `T=0,05`. Confirmado a escala completa preregistrada (n=8.000): **+7,81 pp**, fix/break 671/46, ratio 14,59, p≈0. Desplegado en `identify_service.py` tras backup.

`calibration.json` se reajustó sobre el corpus oficial n=12.788 bajo el pipeline de decisión vivo, dando **90,54%** especie / 92,43% género / 93,83% familia — confirmado contra 500 llamadas HTTP reales al endpoint de producción (90,60%, sin brecha offline/online). Una cosecha el mismo día de 370 observaciones de especies sin cobertura de evaluación previa (exclusión honesta por id de observación) puntuó 93,24% por sí sola y se fusionó en el corpus oficial (n=12.788→13.158) tras validación del operador, moviendo la línea base citable a **90,61%** especie / 92,54% género / 93,91% familia.

### 4.19 Campaña de calidad en fanerógamas, cosecha del rezago de evaluación, y tres bugs de "métrica congelada" corregidos (11/12-sep-2026)

**Fanerógamas.** Cuatro especies visualmente similares — *Posidonia oceanica*, *Cymodocea nodosa*, *Zostera marina* y *Nanozostera noltii* (incorporada al grupo en esta campaña) — recibieron un tratamiento combinado de calidad y pureza visual: purga de fotos de referencia por debajo de una puntuación de calidad de observación de 8,0 (grado research, acuerdo entre identificadores, bonus de curador de confianza, resolución; rango 0–13), relleno multi-fuente (Minka, iNaturalist, GBIF, DORIS, WoRMS, OBIS, Wikimedia) al mismo umbral, y un clasificador de pureza visual MiniCPM-V en dos fases (sujeto puro / sujeto mixto / otro) que solo busca reemplazo para las fotos que fallan. Un vaivén inicial 70%→40% en la exactitud held-out de una especie entre dos medidas del mismo día se rastreó, tras descargar e inspeccionar manualmente las fotos discrepantes, a un cambio en el *pool held-out en sí* (mañana: fotos del backup legado, implícitamente curadas; tarde: muestra realmente aleatoria del pool completo Minka/iNaturalist, incluyendo encuadres atípicos), no a una regresión del pipeline. Bajo un protocolo controlado con pool fresco y sin fuga, *Posidonia oceanica* mejoró en dos mini-evaluaciones independientes (**+6,7 pp** y **+15,0 pp**) y pasó a producción; *Cymodocea nodosa*, con el mismo tratamiento, **empeoró** en staging en ambas medidas (**−6,7 pp** a **−30 pp**) y **no** se desplegó — la hipótesis de trabajo, no confirmada, es confusión visual estructural con *Nanozostera noltii* más que un déficit de calidad o cantidad de foto.

**Cosecha del rezago de evaluación.** La cosecha de evaluación held-out se amplió para paginar también iNaturalist (antes solo Minka) tras descubrir que varias especies sin evaluar tenían una única observación total en Minka, ya consumida como referencia. La cosecha ampliada añadió 956 observaciones held-out nuevas en 280 especies, reduciendo las especies sin cobertura de evaluación de 280 a **64** de 2.989 en el catálogo. Las 64 restantes se consultaron 1 a 1 contra ambas APIs por su recuento total de observaciones research-grade disponibles: en las 64, el total disponible estaba a 2 o menos del número ya consumido como referencia, confirmando agotamiento genuino de la fuente. Una cosecha adicional, centrada en las 67 especies de mayor impacto (tier de mayor prioridad, acierto <60% con menos de 5 observaciones held-out) añadió 143 observaciones más; la exactitud empírica en ese lote nuevo fue 51%, consistente con especies genuinamente difíciles.

**Tres bugs de frescura de métrica** encontrados y corregidos en el panel de administración, todos con la misma causa raíz — una cifra calculada desde un fichero que ya no era el estado vivo — y todos corregidos igual, calculando directamente desde el corpus de evaluación actual: (1) una cifra de exactitud de nivel superior leía un split in-sample 50/50 en vez del corpus out-of-sample; (2) un bloque de exactitud "post-cutover" comparaba dos ficheros congelados de una fecha anterior y luego se sobrescribía sin condición por un segundo overlay que leía otro fichero congelado distinto y nunca recalculaba; (3) la selección del fichero de inventario de cosecha elegía el nombre de fichero alfabéticamente último en vez del de timestamp interno más reciente, sirviendo silenciosamente un inventario pre-fusión obsoleto. Los tres son ahora cálculos en vivo de fuente única contra el corpus de evaluación actual (n=18.273 tras las dos cosechas anteriores, exactitud de especie **86,85%** out-of-sample — bajando desde el 88,55% pre-cosecha no porque nada haya empeorado, sino porque unas 225 especies pasaron de "sin datos de evaluación" a "datos de evaluación reales, a menudo con n bajo" en la misma pasada, que es el efecto buscado de la cosecha y se reporta aquí en vez de suavizarlo).

### 4.20 Cierre de nomenclatura del catálogo, campaña de calidad de galería a escala, y eval_n de fanerógamas ampliado a n=60 (12–14-sep-2026)

**Auditoría de nomenclatura.** Se construyó una herramienta canónica de auditoría taxonómica (WoRMS AphiaID como fuente primaria, GBIF Backbone Taxonomy como fallback universal) validada en rondas ciegas antes de tocar el catálogo. Sobre las 2.989 especies: **85 renombres** aplicados (nombre científico desactualizado → nombre aceptado, almacenado en un campo `accepted_name` nuevo sin tocar el identificador técnico que usan `patterns/`/FAISS, tras un incidente real donde una primera implementación sí lo tocaba y desalineaba silenciosamente ~29 especies del recuento de fotos/evaluación — detectado en menos de 2 minutos por una verificación obligatoria post-cambio, revertido, corregido por diseño) y **4 fusiones de slug duplicado** (dos identificadores técnicos distintos resolviendo al mismo taxón aceptado, p. ej. por una variante ortográfica histórica) completadas de extremo a extremo — fotos SSD+archivo fusionadas sin pérdida (verificación byte a byte por colisión de nombre de fichero), catálogo actualizado a 2.985 especies, reembebido, FAISS reconstruido.

**Campaña de calidad de galería a escala (Q≥8).** Extendiendo el tratamiento de calidad de §4.19 más allá de las fanerógamas: se comprobaron 66+140+246 especies candidatas (tier0 con acierto <40%, galería fina <60%, y una lista más amplia <60% sin probar antes) y se aplicaron sustituciones reales donde había recambio de calidad ≥8,0 disponible en Minka/iNaturalist. Yield muy desigual según la composición del lote — 5/96 en los dos primeros lotes (mayoría ya agotada en las fuentes actuales) frente a **132/212 (62%)** en el tercero, con **7.487 sustituciones de foto reales aplicadas**, reembebido de las especies afectadas, y reconstrucción de FAISS (848.554→848.883 vectores). Un incidente real durante esta campaña —una lista de especies pasada como variable de shell a través de dos capas de SSH anidadas se vació silenciosamente, y como `species = args.species or CATÁLOGO_COMPLETO` (lista vacía es falsy en Python), el proceso de sustitución real corrió sin restricción contra el catálogo entero durante ~35 minutos antes de ser detectado y detenido— se documenta por transparencia: el daño fue el mismo tipo de operación seguramente reversible (sustituir foto de peor por mejor calidad) aplicado a 9 especies fuera del alcance previsto, sin pérdida de datos ni corrupción, corregido en la misma sesión con el fix de causa raíz aplicado a partir de entonces (nunca listas de especies interpoladas a través de SSH anidado).

Un hallazgo de eficiencia real durante esta campaña: el algoritmo original siempre paginaba el pool completo de observaciones *research-grade* del taxón (a veces miles) solo para poder puntuar las 100–300 fotos que ya había en disco. Sustituido por una consulta puntual por identificador (agrupada de 200 en 200) para puntuar solo lo que ya se tiene, con salida temprana total cuando nada está por debajo del umbral de calidad, y una caché local de 24h para el pool completo cuando sí hace falta buscarlo — verificado contra el algoritmo original en casos reales extraídos del propio log de producción (no simulados), resultados idénticos. Las especies cuya galería ya cumplía el umbral de calidad pasaron de 25–60s a ~1s de procesamiento; las que necesitaban el pool completo, sin cambio en la primera pasada pero ~25× más rápidas en re-ejecuciones sobre el mismo taxón gracias a la caché.

**Eval_n de fanerógamas ampliado de n≈15–20 a n=60.** Continuando el tratamiento de §4.19: una segunda cosecha dirigida a las 4 especies de fanerógamas añadió 160 observaciones held-out nuevas (40 por especie, sin solape con las anteriores, verificado por deduplicación contra la galería). Con n=60 —una base bastante más fiable que n=15–20— la lectura cambia de forma no trivial y en direcciones opuestas para dos especies del grupo: ***Posidonia oceanica* pasa de 55,0% a 73,3%** (confirma, con muestra suficiente, una estimación de mini-evaluación anterior que se había dado por no confirmada con n=20), mientras que ***Cymodocea nodosa* pasa de 80,0% a 70,0%** (empeora, en dirección contraria a Posidonia — no es ruido de muestra pequeña, refuerza la decisión de §4.19 de no llevarla a producción). *Nanozostera noltii* sube de 80,0% a 86,7%; *Zostera marina* baja de 80,0% a 75,0%. Con los datos ampliados, se recalcularon las confusiones cruzadas dentro del grupo de 4 especies: **zostera_marina↔nanozostera_noltii muestra 20 confusiones en 120 muestras (13/60 + 7/60)** — evidencia sustancialmente más fuerte que el 2/43 insuficiente descartado en una revisión anterior para el grupo completo de 4 — y se añadió el par a la regla de abstención por indistinguibilidad visual experta (contribución 4, §1.5); las demás combinaciones cruzadas del grupo (cymodocea→posidonia, cymodocea→nanozostera, 2/60 cada una) siguen sin evidencia suficiente y no se tocaron.

Nota de honestidad metodológica: la exactitud OOS agregada del catálogo completo permanece estable (85,85–85,86% a lo largo de esta campaña) pese a los cambios anteriores — consistente con el patrón ya documentado en §4.19, donde ampliar la cobertura de evaluación y corregir nomenclatura no mueve por sí solo la métrica de acierto agregada; el trabajo de esta sección es principalmente higiene de catálogo y de evaluación, no una nueva ganancia de modelo. La cifra de acierto del Resumen (§0, 75,97–79,25% según cohorte) corresponde al protocolo `harvest_calib` estratificado por observación sobre una cohorte fija anterior y **no se ha vuelto a ejecutar sobre esa misma cohorte** desde esta campaña; reconciliarla con el 85,85% del panel en vivo (que usa una cohorte de evaluación distinta y creciente, `calib_raw_t05`, no la cohorte `harvest_calib` original) requeriría una cosecha dedicada bajo el protocolo original antes de poder comparar los dos números directamente — queda pendiente, señalado aquí en vez de mezclar ambos sin más.
...
*Paper en preparación. Versión 2026-09-01. Traducción de la versión en inglés (`01_biofauna.md`), que es la referencia autorizada en caso de discrepancia. Revistas objetivo: Biodiversity Data Journal, PeerJ, o Ecological Informatics.*
