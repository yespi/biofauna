...
We present BioFauna, a deep learning system for automated identification of Mediterranean marine fauna from photographs. The production system uses **BioCLIP-2.5 ViT-H** (632M parameters, 1024-dimensional embeddings) as a **frozen** vision encoder, followed by **k-nearest neighbors** (k=15) with a prototype-similarity boost and a multiplicative geographic prior over a gallery of **785,897 image embeddings across 4,702 target species**, **test-time augmentation** (query + 90% center crop), hierarchical taxonomic abstention, and logistic confidence calibration. It runs on consumer hardware (NVIDIA RTX 3060 12GB) and is deployed on the FotoFauna citizen science platform, where it also drives an hourly automated-identification ("AutoID") pipeline that publishes high-confidence results directly to the Minka citizen-science network.

On an out-of-sample, observation-stratified, leak-checked calibration set (`harvest_calib`, n=12,788), the single-photo-per-observation system with test-time augmentation achieved **75.97% top-1 species accuracy**, **81.29% genus**, and **84.90% family** (August 2026). Subsequent inference-time mechanisms (§4.9–§4.14) raised that figure to **77.77%** without training. Catalog densification of under-represented species, still with a frozen encoder (§4.15), then raised the 1-Sep densification harvest to **79.25%** (Tier-1 75.10%) at **785,897** gallery embeddings. The trusted operational jsonl on the same n=12,788 cohort is **79.10%** (10,115 hits; a 19-photo count difference vs the autopsia tally). A production-only FAISS-index / label-array desynchronization that briefly assigned marine names to terrestrial queries at 85–100% confidence was diagnosed and fixed without invalidating any disk-based evaluation (§4.16). A 4-Sep staging densification campaign (night85) overlays extra gallery vectors **without writing live FAISS**; same-scorer McNemar reports **282 fixed / 80 broken** (+1.58 pp, ratio 3.53) and is **not** cut over, because the replica omits live Cubo B / geo (§4.17). High-confidence predictions (calibrated probability ≥ 0.80) reach an estimated **95.3% precision** at **57.4% coverage**, the operating point used for automated publication. Scaling from BioCLIP ViT-L to ViT-H contributed the largest single gain (+6.8pp species accuracy on an earlier, smaller cohort); k-NN tuning, a full-catalog re-embedding, and test-time augmentation each contributed further, smaller gains. Where an observation carries more than one photo (25.1% of the calibration set), a zero-training late-fusion of per-photo k-NN scores raises accuracy on the full corpus to **76.76% species** (+0.79pp) and to **84.70%** (+3.15pp) on the multi-photo subset alone (§4.7); this is now the production behavior. A systematic search for additional gains through model fine-tuning — a full-backbone LoRA fine-tune, a frozen-backbone linear projection head, embedding outlier filtering, abstention-margin retuning, a biologically-motivated re-ranking heuristic, and a frozen-backbone Supervised Contrastive re-ranker scoped to the hardest known confusion pairs, the last evaluated under a pre-registered kill-switch protocol — did **not** improve the out-of-sample species metric beyond what test-time augmentation already achieves; we report all of these as negative results with quantified costs, since a rigorous account of what does not work is, in a project maintained by a single practitioner without a dedicated ML research team, as valuable as what does.
...
1. **ViT-H gallery re-embedding at full catalog scale** for Mediterranean marine fauna — **785,897** embeddings across **4,702** target species as of 2026-09-01 (762,082 / 4,709 as of the 2026-08-27 TTA baseline), unifying a previously split SSD/HDD-archive photo store and later densifying photo-starved taxa (§4.15).
...
9. **An operational invariant for retrieval indexes** (§4.16): after a hot `/reload`, the FAISS index and its row-aligned `species_ids` must be loaded together; rebuilding only the in-memory label array from current `embeddings.npy` silently maps correct neighbors to the wrong species names.
10. **Staging densification with an overlay McNemar** (§4.17): extra gallery vectors can be scored against production FAISS as a CPU mmap overlay (one index per process) without a live cutover; the overlay-vs-control N/M is not a live-identify delta when the replica omits Cubo B / geo.
...
### 4.15 Catalog densification without a new encoder (2026-08-31 – 2026-09-01)

After the inference-time stack of §4.9–§4.14 was frozen at **77.77%** species (n=12,788) — five further architectural hypotheses returned no significant gain — remaining error was attacked with **more reference photos**, not more machinery. Targeted harvests into photo-starved species (Tier-3 colour-group cleanup, then successive Tier-1 lots, then a mass lot) were re-embedded with the same frozen ViT-H encoder and rebuilt into FAISS. Production after the mass lot: **785,897** embeddings / **4,702** species.

Official `harvest_calib` on the same n=12,788 cohort: **79.25%** species / **75.10%** Tier-1. The gain is densification of the gallery, not a new scorer. An overnight increment on 726 additional species (staging index **807,267**) measured **79.10%** globally (−0.15pp, McNemar p=0.053) despite a **+1.73pp** lift on the 726-species slice itself (p=0.014): k-NN interference against untouched neighbors. That staging index was **not** cut over. A `score / log1p(n_ref)` reweight was likewise discarded (−0.07pp). We therefore treat **79.25% / 785,897** as the live official baseline, and 807,267 as an artifact not to promote.

A detector-crop experiment on the query (YOLO26n COCO, pad=0.25) scored **78.74%** on the same official set — below 79.25% — so default FotoFauna PRE cropping remains the existing YOLO-COCO path, not a marine-prompted YOLOE crop (which recovers marine boxes but lowers BioFauna species accuracy after the crop, 78.3%→52–56% on a 23-photo marine harvest).

As of 3 Sep 2026 the operational source of truth is the jsonl `calib_raw.jsonl`: **10,115 / 12,788 = 79.10%** species (Tier-1 **74.88%**, n=7,282). The 79.25% figure is the 1-Sep autopsia count of the same harvest (Δ19 photos). We keep 79.25% as the densification-harvest row in this section and **79.10% jsonl** as the live official baseline.

### 4.16 Production FAISS / label-array desynchronization (found 2026-09-01, fixed)

**Symptom.** For roughly 34 hours (31 Aug ~12:03 to 1 Sep 22:42 CEST), live `/identify` assigned **marine species names at 85–100% calibrated confidence to terrestrial photographs** (e.g. a rockrose identified as *Chthamalus stellatus*). AutoID publications that used the live service in that window are not trustworthy; disk-based evaluation jobs were unaffected.

**Cause.** The FastAPI process had loaded a FAISS index of **777,544** vectors at boot. Subsequent `POST /reload` calls rebuilt the in-memory label array `KY` from the then-current `embeddings.npy` (**785k-row layout**) **without reloading FAISS**. Retrieval therefore returned the correct neighbors of the old index and translated them with the **wrong row names**. Offline replay of observation 75314: aligned 777k index + matching `species_ids.npy` → *Cistus albidus* (sim≈0.94); the same FAISS rows with `KY` from the 785k layout → *Chthamalus stellatus*, matching production. This is not a Minka/iNaturalist taxon-id swap and not a corrupt re-embedding: `embeddings.npy` on disk stayed row-aligned with `species_ids.npy`.

**Why official metrics still hold.** `harvest_calib`, McNemar gates, the 79.25% / 79.10% staging comparison, and the pad=0.25 crop experiment all open FAISS **from disk** with `species_ids.npy`. They never consulted the live `KY`. Fine-tuning ablations likewise do not call `:8090`.

**Fix (in production).** Neighbor translation uses the FAISS-accompanying `species_ids.npy` (`_faiss_y`), not `KY`. `load_faiss()` refuses a load when `len(ids) != ntotal`. `/reload` now calls `load_faiss()`. `/health` exposes `faiss_ntotal`, `faiss_ids`, and `faiss_aligned`. After restart: `faiss_ntotal=785897`, `faiss_ids=785897`, `faiss_aligned=true`. Six terrestrial Minka observations that had been marine magnets recovered their botanical/terrestrial names; 49/50 official-calibration hits still matched live top-1.

The lesson is operational, not architectural: **a retrieval index and its row labels are one artifact**. Hot-reloading prototypes without the index is a silent, high-confidence failure mode.

### 4.17 Staging densification overlay (night85, 2026-09-03–04, not cut over)

Live identify remains **79.10%** jsonl on FAISS **785,897** (`faiss_aligned=true`). A reversible harvest of close-call, photo-starved species (iNaturalist + Minka, **local archive JPGs first**, embed via `POST /embed`) writes extra vectors under `exp_density_rank2/` and **does not** mutate `dataset/faiss_index/` or `patterns/`.
...
### 4.20 Catalog nomenclature closure, gallery quality campaign at scale, and seagrass eval_n expanded to n=60 (2026-09-12/14)

**Nomenclature audit.** A canonical taxonomic audit tool (WoRMS AphiaID as primary source, GBIF Backbone Taxonomy as universal fallback) was built and validated in blind rounds before touching the catalog. Across the 2,989-species catalog: **85 renames** applied (outdated scientific name → accepted name, stored in a new `accepted_name` field without touching the technical identifier used by `patterns/`/FAISS, after a real incident where a first implementation did touch it and silently desynced ~29 species' photo/eval counts — caught within 2 minutes by a mandatory post-change verification, reverted, fixed by design) and **4 duplicate-slug merges** (two distinct technical identifiers resolving to the same accepted taxon, e.g. a historical spelling variant) completed end-to-end — SSD+archive photos merged without loss (byte-level verification on filename collisions), catalog updated to 2,985 species, re-embedded, FAISS rebuilt.

**Gallery quality campaign at scale (Q≥8).** Extending the quality treatment of §4.19 beyond seagrasses: 66+140+246 candidate species were checked (tier0 with <40% accuracy, thin gallery <60%, and a broader untested <60% list) and real substitutions applied where a quality-≥8.0 replacement was available on Minka/iNaturalist. Yield varied sharply by batch composition — 5/96 in the first two batches (mostly already exhausted in current sources) versus **132/212 (62%)** in the third, with **7,487 real photo substitutions applied**, re-embedding of affected species, and FAISS rebuild (848,554→848,883 vectors). A real incident during this campaign — a species list passed as a shell variable through two layers of nested SSH silently came back empty, and since `species = args.species or FULL_CATALOG` (an empty list is falsy in Python), the real-substitution process ran unconstrained against the entire catalog for ~35 minutes before being caught and stopped — is documented for transparency: the damage was the same, safely reversible operation (swap a worse photo for a better one) applied to 9 species outside the intended scope, with no data loss or corruption, corrected within the same session with the root-cause fix applied from then on (never interpolate species lists through nested SSH again).

A real efficiency finding during this campaign: the original algorithm always paged the taxon's entire research-grade observation pool (sometimes thousands) just to score the 100–300 photos already on disk. Replaced with a point lookup by ID (batched 200 at a time) to score only what is already held, with a full early exit when nothing is below the quality threshold, and a 24h local cache for the full pool when it is actually needed — verified against the original algorithm on real cases pulled from the production log itself (not simulated), identical results. Species whose gallery already met the quality bar went from 25–60s to ~1s of processing; species needing the full pool were unchanged on first pass but ~25× faster on re-runs against the same taxon thanks to the cache.

**Seagrass eval_n expanded from n≈15–20 to n=60.** Continuing the treatment of §4.19: a second targeted harvest for the 4 seagrass species added 160 new held-out observations (40 per species, no overlap with prior ones, verified by dedup against the gallery). At n=60 — a substantially more reliable base than n=15–20 — the reading shifts non-trivially and in opposite directions for two species in the group: ***Posidonia oceanica* moves from 55.0% to 73.3%** (with a sufficient sample, this confirms an earlier mini-eval estimate that had been reported as unconfirmed at n=20), while ***Cymodocea nodosa* moves from 80.0% to 70.0%** (worsens, in the opposite direction from Posidonia — not small-sample noise, reinforcing the §4.19 decision not to ship it to production). *Nanozostera noltii* rises from 80.0% to 86.7%; *Zostera marina* falls from 80.0% to 75.0%. With the expanded data, cross-confusions within the 4-species group were recomputed: **zostera_marina↔nanozostera_noltii shows 20 confusions in 120 samples (13/60 + 7/60)** — substantially stronger evidence than the insufficient 2/43 dismissed in an earlier review for the full 4-species group — and the pair was added to the expert visual-indistinguishability abstention rule (contribution 4, §1.5); the other cross-combinations within the group (cymodocea→posidonia, cymodocea→nanozostera, 2/60 each) remain without sufficient evidence and were left untouched.

Methodological honesty note: aggregate catalog-wide OOS accuracy stayed stable (85.85–85.86%) throughout this campaign despite the above changes — consistent with the pattern already documented in §4.19, where expanding evaluation coverage and fixing nomenclature does not by itself move the aggregate accuracy metric; the work in this section is primarily catalog and evaluation hygiene, not a new model gain. The headline accuracy figure in the Abstract (§0, 75.97–79.25% depending on cohort) corresponds to the observation-stratified `harvest_calib` protocol on a fixed earlier cohort and **has not been re-run on that same cohort** since this campaign; reconciling it with the 85.85% shown on the live panel (which uses a different, growing evaluation cohort, `calib_raw_t05`, not the original `harvest_calib` cohort) would require a dedicated harvest under the original protocol before the two numbers can be compared directly — left open here rather than conflating the two without qualification.
...
*Paper in preparation. Version 2026-09-01. Target journals: Biodiversity Data Journal, PeerJ, or Ecological Informatics.*
